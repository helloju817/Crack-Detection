...
tf.app.flags.DEFINE_string('model', 'enhancenet', '')
tf.app.flags.DEFINE_string('image_path', '/where/your/saved/image/folder', '')
tf.app.flags.DEFINE_integer('batch_size', 128, '32')
tf.app.flags.DEFINE_integer('SR_scale', 8, '4')
tf.app.flags.DEFINE_integer('num_readers', 4, '')
tf.app.flags.DEFINE_integer('input_size', 64, '32')
tf.app.flags.DEFINE_float('learning_rate', 0.0001, 'define your learing strategy 0.0001')
tf.app.flags.DEFINE_float('moving_average_decay', 0.997, '')
tf.app.flags.DEFINE_string('vgg_path', 'vgg_19.ckpt', '/where/your/vgg_19.ckpt')
tf.app.flags.DEFINE_integer('num_workers', 4, '')
tf.app.flags.DEFINE_integer('max_to_keep', 10, 'how many do you want to save models?')
tf.app.flags.DEFINE_integer('save_model_steps', 15000, '')
...
def main(argv=None):
    ...
    ######################### Model setup
    low_size = int(FLAGS.input_size / FLAGS.SR_scale)
    high_size = FLAGS.input_size
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
        ...
        # plt.plot(range(1,10001), generator_loss_val, 'b--')
        # plt.title('loss')
        # plt.savefig('loss__graph.png')
        print('')
        print('*' * 30)
        print(' Training done!!! ')
        print('*' * 30)
...
if __name__ == '__main__':
    tf.app.run() #cmd에서 입력한 argument들을 parsing해서 hyperparameter로 사용
